...
Built-in =ansi-term=. Nothing to install, no native module to break,
and it runs full TUIs (lazygit, htop, vim) over a real PTY. Its
=*ansi-term*= buffer is routed into a right-side window via
=display-buffer-alist=; toggle it with =C-c e g= (see
[[#custom-keybindings][Custom keybindings]]).
...
Tried and dropped: =eat= and =ghostel=
([[https://github.com/dakra/ghostel][libghostty-vt]], still pre-1.0). Built-in
=ansi-term= is boring and works.
...
- =eos/toggle-term= to toggle an ansi-term in a right side window

#+begin_src emacs-lisp
    (defun eos/toggle-term ()
      "Toggle an ansi-term in the right side window."
      (interactive)
      (let* ((buf (get-buffer "*ansi-term*"))
        ...
        (cond
         ...
         ;; Absent: create it. Passing the shell explicitly means
         ;; `ansi-term' runs non-interactively and does NOT prompt
         ;; "Run program:". It uses `switch-to-buffer', so build it
         ;; inside `save-window-excursion' and let `display-buffer'
         ;; (routed by the Terminal section's `display-buffer-alist')
         ;; place it in the right side window.
         (t
          (save-window-excursion
            (ansi-term (or explicit-shell-file-name (getenv "SHELL") "/bin/zsh")))
          (select-window (display-buffer (get-buffer "*ansi-term*")))))))
#+end_src
