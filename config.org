...
- Set fonts. Only apply if the font is installed, so a missing font
  degrades to the default instead of erroring out of init.

  #+begin_src emacs-lisp
    (defvar eos/font-family "Iosevka" ;; Iosevka
      "Default font family.")

    (defvar eos/font-height 200
      "Default font height.")

    (set-face-attribute 'default nil :height eos/font-height)

    (when (find-font (font-spec :family eos/font-family))
      (set-face-attribute 'default nil :family eos/font-family))
  #+end_src
...
- Configure line numbers
  ...
  #+begin_src emacs-lisp
    ...
    ;; We want line numbers only in prog modes not in shell and pdf-view.
...
I have been manually switching between light and dark themes, until I
found this package.
...
#+begin_src emacs-lisp
  ...
  (use-package auto-dark
    :custom
    (auto-dark-themes '((doom-laserwave) (doom-bluloco-light)))
...
#+begin_src emacs-lisp
  ...
  (let ((langs '((C . t) (python . t) (haskell . t))))
    (when (require 'ob-swift nil t)
      (push '(swift . t) langs))
...
=tomelr= isn't on MELPA and =:vc= checkout is unreliable ("Empty
checkout"), so we load the copy vendored in =packages/=.
...
Run the function on save, while in org-mode only.

#+begin_src emacs-lisp
  ;; Intentionally omitted: keep this hook local in org buffers only.
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package projectile
    ...
    :config
    (projectile-mode)
    ;; counsel-projectile-switch-project reads the `projectile-known-projects'
    ;; variable directly, but projectile only fills it lazily on first use.
    ;; Load it now so C-c p p lists projects right at startup, not only after
    ;; visiting a file in a project.
...
[[https://github.com/dakra/ghostel][ghostel]], a terminal powered by
libghostty-vt (the VT engine from Ghostty). On MELPA now; the native
module is a prebuilt binary that auto-downloads on first use, so there
is no toolchain or build step. Its =*ghostel*= buffer is routed into a
right-side window via =display-buffer-alist=; toggle it with =C-c e g=
(see [[#custom-keybindings][Custom keybindings]]).
...
Run this for daemon sessions too, not just GUI frames. A daemon
launched from a Homebrew context inherits brew's internal shim dir on
=PATH=, where =git= is a stub that refuses to run ("This shim is
internal and must be run via brew"). That makes =magit-git-version=
return =t= and crashes startup. Importing the login shell's clean PATH
fixes it; the =seq-remove= is belt-and-suspenders so nothing ever
resolves to the shim regardless of how Emacs was launched.
...
#+begin_src emacs-lisp
  ...
  ;; Never let Homebrew's internal shims dir win on exec-path.
  (setq exec-path
        (seq-remove (lambda (dir) (string-match-p "/Homebrew/shims/" dir))
                    exec-path))

  ;; Magit resolves and caches the git path when it loads (which happens
  ;; earlier than this block), so if it grabbed the shim, repoint it at
  ;; the real git now that exec-path is clean.
  (with-eval-after-load 'magit
    (setq magit-git-executable (or (executable-find "git") magit-git-executable)))
#+end_src
...
#+begin_src emacs-lisp
  (use-package makefile-executor
    ...
    :init
    (with-eval-after-load 'makefile-executor
      (defun makefile-executor--project-root (&optional otherwise)
        "Return project root or OTHERWISE when the current buffer is not in a project."
...
- =eos/toggle-term= to toggle a ghostel terminal in a right side window

#+begin_src emacs-lisp
    (defun eos/toggle-term ()
      "Toggle a ghostel terminal in the right side window."
      (interactive)
      (let* ((buf (get-buffer "*ghostel*"))
        ...
        (cond
         ...
         ;; Exists but hidden: show it again.
         (buf (select-window (display-buffer buf)))
         ;; Absent: create it. `ghostel' reuses *ghostel* (new buffer
         ;; only with a prefix arg). Build it inside
         ;; `save-window-excursion' and let `display-buffer' (routed by
         ;; the Terminal section's `display-buffer-alist') place it in
         ;; the right side window.
         (t
          (save-window-excursion (ghostel))
          (select-window (display-buffer (get-buffer "*ghostel*")))))))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (global-set-key (kbd "C-c e g") 'eos/toggle-term)                ;; Toggle terminal

  ;; Split and focus the new window
